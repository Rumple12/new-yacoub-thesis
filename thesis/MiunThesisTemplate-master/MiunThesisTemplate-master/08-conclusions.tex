...
\textbf{RQ1} asked how accurately the deterministic n8n workflow produced the expected fan action for the defined temperature cases. The deterministic workflow produced the expected result for both tested cases: the high-temperature case led to \texttt{fan\_on}, and the low-temperature case led to \texttt{fan\_off}. This evidence supports the deterministic baseline for the defined scenario and run count reported in Chapter~\ref{sec:results}.

\textbf{RQ2} asked how accurately the minimal agent-enhanced workflow produced contract-valid action output compared with the deterministic baseline for the defined cases. The agent-enhanced workflow produced structured, contract-shaped output for the same defined cases and routed the resulting actions through the same middleware boundary. The result shows that an LLM decision step can be integrated into the workflow-to-action pipeline while preserving the expected action structure for the implemented cases.
